...
const HomeLayout = () => {
  ...
  const handleJobDetailsSubmit = (data: IJobDetails) => {
    setJobDetails(data);
  };

  const handleInterviewSettingsSubmit = (data: IInterViewSettings) => {
    setInterviewSettings(data);
  };

  const handleIRequisitionDetailsSubmit = (data: IRequisitionDetails) => {
      ...
      <Container maxW="1200px">
        ...
        <Tabs index={page} isLazy lazyBehavior="keepMounted">
          ...
          <Grid display="grid" gridTemplateColumns="3fr 2fr" gap="24px">
            <TabPanels>
              <TabPanel>
                <RequisitionForm handleTab={handlePage} setRequisitionForm={handleIRequisitionDetailsSubmit} />
              </TabPanel>
              <TabPanel>
                <JobDetailsForm handleTab={handlePage} setJobForm={handleJobDetailsSubmit} />
              </TabPanel>
              <TabPanel>
                <InterviewSettingsForm handleTab={handlePage} setInterviewForm={handleInterviewSettingsSubmit} />
              </TabPanel>
            </TabPanels>
